...
plt.ylabel('time [$kyr$]', fontsize= fs)
plt.title("time between all gypsum and first halite", fontsize= fs*1.2)
...
for conf in [0,1,2]:
    for iie in [1, 5]:
        for iic in [0, -1]:
            ...
            # ax.plot(G, y2 , color = color_conf[conf], linewidth = lw)
            axins.plot(G, y1 , color = color_conf[conf], linewidth = lw)
            # axins.plot(G, y2 , color = color_conf[conf], linewidth = lw)
